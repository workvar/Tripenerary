...
export const type = {
  ...
  label: { fontSize: 11, fontWeight: '700', letterSpacing: 1.1, color: colors.accent },
} as const satisfies Record<string, TextStyle>;

const elevate = (opacity: number, blur: number, y: number, android: number): ViewStyle => ({
  shadowColor: '#1B1D1C',
...
export const motion = { fast: 180, base: 280, slow: 460 } as const;

export const hairlineWidth = Platform.OS === 'android' ? 1 : 0.5;
export const hairline = { height: hairlineWidth, backgroundColor: colors.borderSoft } as const;
