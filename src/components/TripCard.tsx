...
import type { TripRecord, TripState } from '@/types';

const BADGE: Record<TripState, { readonly backgroundColor: string; readonly color: string }> = {
...
function dateRange(trip: TripRecord): string {
  if (!trip.startDate || !trip.endDate) return 'Not loaded yet';
  const from = formatDate(trip.startDate, { day: 'numeric', month: 'short' });
  const to = formatDate(trip.endDate, { day: 'numeric', month: 'short', year: 'numeric' });
  return `${from} – ${to}`;
}
  ...
  return (
    <Animated.View style={{ opacity: enter, transform: [{ translateY }] }}>
      ...
      >
        ...
        <View style={s.top}>
          <View style={[s.badge, { backgroundColor: badge.backgroundColor }]}>
            <Text style={[s.badgeText, { color: badge.color }]}>{status.label}</Text>
          </View>
