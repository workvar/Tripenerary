...
export default function LandingScreen({ library, onOpenSettings }) {
  const { trips, status, addTrip, removeTrip, openTrip, refreshAll } = library;
  const [sheetOpen, setSheetOpen] = useState(false);

  const ordered = useMemo(() => sortTrips(trips), [trips]);
  const anyRefreshing = Object.values(status).some((v) => v && v.refreshing);
  const lastSynced = trips.reduce((a, t) => (t.syncedAt > (a || '') ? t.syncedAt : a), null);
  ...
  return (
    <SafeAreaView style={s.safe} edges={['top']}>
      <View style={s.header}>
        ...
        <IconButton glyph={'\u{21BB}'} onPress={refreshAll} spinning={anyRefreshing} />
        <View style={s.gap} />
        <IconButton glyph={'\u{2699}'} onPress={onOpenSettings} />
      </View>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={anyRefreshing} onRefresh={refreshAll} tintColor={colors.primary} />
        }
      >
        {ordered.length === 0 ? (
          ...
          <View style={s.list}>
            {ordered.map((trip, i) => (
              <TripCard
                ...
                trip={trip}
                index={i}
                busy={status[trip.id] && status[trip.id].refreshing}
                error={status[trip.id] && status[trip.id].error}
                onPress={() => openTrip(trip.id)}
                onLongPress={() => confirmRemove(trip)}
              />
            ))}
            <Text style={s.footer}>{'Last refreshed ' + formatSyncedAt(lastSynced) + ' · hold a card to remove'}</Text>
          </View>
        )}
      </ScrollView>

      <Press style={s.fab} onPress={() => setSheetOpen(true)} scaleTo={0.9}>
...
const s = StyleSheet.create({
  ...
  emptyBody: { ...type.body, color: colors.textMuted, textAlign: 'center', paddingHorizontal: spacing.xl },
  emptyBtn: {
