...
export default function SplashScreen({ onDone, ready }) {
  ...
  useEffect(() => {
    Animated.sequence([
      ...
      Animated.parallel([
        Animated.spring(mark, { toValue: 1, useNativeDriver: true, damping: 12, stiffness: 120, mass: 0.9 }),
        Animated.timing(spin, { toValue: 1, duration: 1100, easing: Easing.inOut(Easing.cubic), useNativeDriver: true }),
      ]),
      Animated.timing(word, { toValue: 1, duration: 420, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
    ]).start();
  }, [bloom, mark, spin, word]);

  // Leave only once the library has booted and the minimum showing time is up.
  useEffect(() => {
    if (!ready || finished.current) return;
    finished.current = true;
    const t = setTimeout(() => {
      Animated.timing(exit, { toValue: 1, duration: 420, easing: Easing.in(Easing.cubic), useNativeDriver: true })
        .start(({ finished: ok }) => ok && onDone());
    }, SPLASH_MIN_MS);
  ...
  return (
    <Animated.View style={[s.screen, screenStyle]}>
      ...
      <Animated.View style={[s.words, wordStyle]}>
