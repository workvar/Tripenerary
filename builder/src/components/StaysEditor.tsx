      ...
      {draft.stays.map((stay) => (
        <div key={stay.id} className="card space-y-3">
          <div className="flex items-center gap-2">
            ...
            <button type="button" className="btn-mini hover:border-danger hover:text-danger" onClick={() => remove(stay.id)}>
              {'\u{2715}'}
            </button>
          </div>
          ...
          <Area label="Notes" value={stay.notes} onChange={(v) => patch(stay.id, { notes: v })} rows={2} />
